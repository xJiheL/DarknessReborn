Shader "Custom/Shader_Waterfall" {
	Properties {
		[Header (Water)]
...
//		}
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			uvWaterTex = float2 (IN.uv_MainTex.x + _Time.x * _SpeedTex.x*2, IN.uv_MainTex.y + _Time.x * _SpeedTex.y*2);
			c += tex2D (_MainTex, uvWaterTex*2);
			c = saturate (c);
