...
namespace Invector.vCharacterController
{
    public abstract class vThirdPersonMotor : vCharacter
    {
        ...
        internal Vector2 input;                               // generate input for the controller        
        internal float speed, direction, verticalVelocity;    // general variables to the locomotion
        internal float velocity;                               // velocity to apply to rigdibody
        ...
        protected void StopMove()
        {
            ...
            else if (debugWindow) Debug.DrawLine(transform.position + Vector3.up * (_capsuleCollider.height * 0.5f), transform.position + targetDirection.normalized * (_capsuleCollider.radius * 0.2f), Color.blue, 0.01f);
            ...
            if (_capsuleCollider != null)
            {
                ...
                if (groundCheckMethod == GroundCheckMethod.High)
                {
                    ...
                    if (Physics.SphereCast(ray, radius, out groundHit, _capsuleCollider.radius + 2f, groundLayer))
                    {
                        ...
                        if (dist > (groundHit.distance - _capsuleCollider.radius * 0.1f)) dist = (groundHit.distance - _capsuleCollider.radius * 0.1f);
        ...
        protected virtual void Sliding()
        {
            ...
            if (GroundAngle() >= slopeLimit + 1 && GroundAngle() <= 85 && groundDistance <= 0.05f)
            {
                isSliding = true;
                isGrounded = false;
        ...
        {            
            ...
            if (Physics.SphereCast(rayStep, _capsuleCollider.radius * 0.9f, out _hit, stepOffsetEnd - stepOffsetStart, groundLayer) && !_hit.collider.isTrigger)
            {
                if (_hit.point.y >= (transform.position.y) && _hit.point.y <= (transform.position.y + stepOffsetEnd))
                {
                    var _speed = Mathf.Clamp(input.magnitude, 0, 1f);
